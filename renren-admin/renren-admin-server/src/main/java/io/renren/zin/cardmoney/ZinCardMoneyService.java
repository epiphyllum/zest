...
public class ZinCardMoneyService {
    ...
    public TDepositResponse deposit(TDepositRequest request) {
        return requester.request(uniqueId(), "/gcpapi/card/deposit", request, TDepositResponse.class);
    }

    // 3101-保证金提取
    public TWithdrawResponse withdraw(TWithdrawRequest request) {
        return requester.request(uniqueId(), "/gcpapi/card/withdraw", request, TWithdrawResponse.class);
    }

    // 3102-卡余额查询
    public TCardBalanceResponse balance(TCardBalanceRequest request) {
        return requester.request(uniqueId(), "/gcpapi/card/querybalance", request, TCardBalanceResponse.class);
    }

    // 3103-卡保证金存提记录查询
    public TCardBondResponse querybond(TCardBondQuery request) {
        return requester.request(uniqueId(), "/gcpapi/card/querybond", request, TCardBondResponse.class);
    }
}
