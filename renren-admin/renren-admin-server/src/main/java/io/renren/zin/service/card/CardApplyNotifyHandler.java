...
@Service
@Slf4j
public class CardApplyNotifyHandler {
    ...
    public void handle(TCardApplyNotify notify) {
        String trxtype = notify.getTrxtype();
        if (trxtype.equals(ZinConstant.CP450)) {
            handleCP450(notify);
        } else if (trxtype.equals(ZinConstant.CP453)) {
            handleCP453(notify);
        } else if (trxtype.equals(ZinConstant.CP458)) {
            handleCP458(notify);
        } else if (trxtype.equals(ZinConstant.CP451)) {
            handleCP451(notify);
        } else if (trxtype.equals(ZinConstant.CP452)) {
            handleCP452(notify);
        } else if (trxtype.equals(ZinConstant.CP462)) {
            handleCP452(notify);
        } else {
    ...
    public void handleCP450(TCardApplyNotify notify) {
        ...
        tx.executeWithoutResult(status -> {
            ...
            if (!notify.getFee().equals(BigDecimal.ZERO) && notify.getState().equals(ZinConstant.CARD_STATE_APPLY_SUCCESS)) {
